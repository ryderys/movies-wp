...
$playlist_relation = streamit_get_playlist_item($playlist_id, 'video');
...
$playlist_url = streamit_get_permalink('video_playlist', $encoded_playlist_id);
$image_url = streamit_placeholder_image();
if (!empty($first_data)) {
    $video_data = streamit_get_video((int) $first_data);
    if (!empty($video_data)) {
        $image_url = !empty($video_data->get_meta('thumbnail_id')) ? wp_get_attachment_image_url($video_data->get_meta('thumbnail_id'), 'full') : streamit_placeholder_image();
    }
}
?>
<div class="col">
            ...
            <div class="d-flex justify-content-between gap-2 mb-1">
                ...
                <div class="dropdown">
                    ...
                    <ul class="dropdown-menu border">
                        <li><a data-playlist-name="<?php echo esc_attr($playlist_name); ?>" data-playlist-id="<?php echo esc_attr($playlist_id); ?>" data-post-type="<?php echo esc_attr('video'); ?>" class="manage_playlist dropdown-item update_user_playlist"><?php echo esc_html__('Update', 'streamit'); ?></a></li>
                        <li><a data-playlist_id="<?php echo esc_attr($playlist_id); ?>" data-post-type="<?php echo esc_attr('video'); ?>" class="dropdown-item delete_user_playlist"><?php echo esc_html__('Delete', 'streamit'); ?></a></li>
                    </ul>
                </div>
            </div>
            <div class="small mb-1">
                <?php echo sprintf(
                    esc_html(_n('%s Video', '%s Videos', $data_count === 0 ? 1 : $data_count, 'streamit')),
