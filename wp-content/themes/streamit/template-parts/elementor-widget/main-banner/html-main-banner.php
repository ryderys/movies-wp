    ...
    <div id="<?php echo esc_attr('home-slider-' . $id_int); ?>"
        ...
        <?php if (!empty($post_data) && is_array($post_data)) :
            foreach ($post_data as $post) {
                ...
                // Full image using helper
                $full_image = streamit_render_image([
                    'attachment_id' => (int) $post->get_meta('thumbnail_id'),
                    'class'         => 'post-img',
                    'alt'           => $post->get_post_name(),
                    'size'          => 'full'
                ]);

                // Portrait image using helper
                $potrait_image = streamit_render_image([
                    'attachment_id' => (int) $post->get_meta('_portrait_thumbmail'),
                    'class'         => 'post-img',
                    'alt'           => $post->get_post_name(),
                    'size'          => 'full'
                ]);

                // Full image URL for background
                $full_image_url = !empty($post->get_meta('thumbnail_id')) ? wp_get_attachment_url($post->get_meta('thumbnail_id')) : esc_url(streamit_placeholder_image());
