...
/**
 * The template for displaying person category single pages
 *
...
global $wp_query;
$person_category_slug = isset($wp_query->query_vars['person_category']) ? sanitize_title($wp_query->query_vars['person_category']) : '';
$st_data = streamit_get_term((string)$person_category_slug, 'person_category');
...
<div class="container-fluid">
    <?php
    // Ensure content_data is set and results exist
    if (isset($content_data) && !empty($content_data->results)) : ?>
        <div class="css_prefix-card-wrapper person_cards grid-view" data-options="yes" data-can-beloaded="1">
            ...
            if (!empty($content_data->results) && ($content_data->maxnumpages > 1))
                echo st_get_load_more_button($content_data->maxnumpages, 'person_single_category', 1, esc_html($load_more_text), esc_html($loading_text), '', '', '', esc_html($term_id));
            ?>
        </div>


    <?php else : ?>
        <p><?php esc_html_e('No Person Found. ', 'streamit'); ?></p>
    <?php endif; ?>
</div>