...
<div class="container-fluid">
        ...
        <div class="css_prefix-card-wrapper movie_cards grid-view" data-options="yes" data-can-beloaded="1">
            <div class="row gy-4 row-cols-2 row-cols-sm-2 row-cols-md-3 row-cols-lg-4 row-cols-xl-5 data-listing">
                <?php foreach ($content_data->results as $st_data) : ?>
                    <?php streamit_get_template('movie/archive/archive_tag_loop.php', ['st_data' => $st_data]); ?>
                <?php endforeach; ?>
            </div>
            <?php
            if ($content_data->maxnumpages > 1) {
                if ($genere_tag_option === 'load_more') {
                    echo st_get_load_more_button($content_data->maxnumpages, 'movie_tag', 1, esc_html($load_more_text), esc_html($loading_text));
                } elseif ($genere_tag_option === 'infinite_scroll') {
                    echo st_get_loader_wheel_container($content_data->maxnumpages, 'movie_tag', 1, '');
                }
            }
            ?>
        </div>
    ...
    <?php else : ?>
        <p><?php esc_html_e('No movie Tag found.', 'streamit'); ?></p>
    <?php endif; ?>
</div>