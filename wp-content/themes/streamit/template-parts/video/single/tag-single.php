...
global $wp_query;
$video_tag_slug = isset($wp_query->query_vars['video_tag']) ? sanitize_title($wp_query->query_vars['video_tag']) : '';
$st_data = streamit_get_term((string)$video_tag_slug, 'video_tag');
...
<div class="container-fluid">
    <?php if (!empty($content_data->results)) : ?>
        <!-- Card Wrapper for Video Display -->
        <div class="css_prefix-card-wrapper  video_cards grid-view " data-options="yes" data-can-beloaded="1">
            ...
            <div class="row gy-4 row-cols-2 row-cols-sm-2 row-cols-md-3 row-cols-lg-4 row-cols-xl-6 data-listing">
                <?php foreach ($content_data->results as $st_data) : ?>
                    <!-- Call to template part to display individual video -->
                    <?php streamit_get_template('video/archive/archive_loop.php', ['st_data' => $st_data]);  ?>
                <?php endforeach; ?>
            </div>
            <?php
            if (!empty($content_data->results) && ($content_data->maxnumpages > 1))
                echo st_get_load_more_button($content_data->maxnumpages, 'video_single_tag', 1, esc_html($load_more_text), esc_html($loading_text), '', '', '', esc_html($term_id));
            ?>
        </div>
    <?php else : ?>
