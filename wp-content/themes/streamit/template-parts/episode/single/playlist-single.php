...
/**
 * The template for displaying a single episode playlist.
 *
...
$playlist_id = isset($_GET['playlist_id']) ? absint($_GET['playlist_id']) : 0;
?>
    ...
    <div class="container-fluid">
        ...
        if (! empty($content_data)) :
            if (get_current_user_id() == $content_data->get_user_id()) :
                ...
                $episodesids = streamit_get_playlist_item($playlist_id, 'episode');
              
                if (! empty($episodesids)) :
                    $display_episode = isset($_GET['episodeid']) && !empty($_GET['episodeid']) ? absint($_GET['episodeid']) : '';
                    $episodes = streamit_get_episodes(['orderby' => 'post__in', 'per_page' => -1, 'include' => $episodesids])->results;

                    if(empty($episodes)){
                        echo '<p class="no_data_found">' . esc_html__('No Episodes found.', 'streamit') . '</p></div></div>';
                        return;
                    }
                    $display_episode_data = null;
                    if (empty($display_episode)) {
                        $display_episode = $episodes[0]->get_id();
                    }
        ?>
                    <div class="row gy-4 flex-column-reverse flex-lg-row-reverse">
                        <div class="col-xxl-3 col-xl-4 col-lg-5">
                            <div class="card">
                                <div class="card-header pb-3 mb-3 border-bottom d-flex align-items-center justify-content-between gap-1">
                                    ...
                                    <small>
                                        <?php
                                        $current_episode_index = array_search($display_episode, array_map(function ($episode) {
                                            return $episode->get_id();
                                        }, $episodes));
                                        if ($current_episode_index !== false) {
                                            echo sprintf(esc_html__('%d/%d', 'streamit'), $current_episode_index + 1, count($episodes));
                                        } else {
                                            echo esc_html__('1/' . count($episodes), 'streamit');
                                        }
                                        ?>
                                    </small>
                                </div>
                                <div class="card-body px-0 pt-0 pb-3">
                                    <div class="playlist-data">
                                        <?php foreach ($episodes as $episode) :
                                            $thumbnail = !empty($episode->get_meta('thumbnail_id')) ? wp_get_attachment_image_url($episode->get_meta('thumbnail_id'), 'full') : streamit_placeholder_image();
                                            $title = esc_html($episode->get_post_title());
                                            $view = esc_html($episode->get_meta('post_views_count'));
                                            $release_date = $episode->get_meta('_episode_release_date');
                                            ...
                                            $redirect_url = streamit_get_permalink('episode_playlist', $encoded_playlist_id);
                                            $redirect_url = add_query_arg('episodeid', $episode->get_id(), $redirect_url);
                                            $active_class = ($display_episode == $episode->get_id()) ? 'active' : '';
                                            if ($active_class === 'active') {
                                                $display_episode_data = $episode;
                                            }
                                        ?>
                                                ...
                                                <div class="playlist-data-card-content">
                                                    ...
                                                    <ul class="playlist-category list-inline d-flex flex-wrap align-items-center m-0 p-0 column-gap-3 row-gap-1">
                                                        ...
                                                        if (isset($streamit_core_options['streamit_show_viewcounter']) && $streamit_core_options['streamit_show_viewcounter'] === 'yes') : ?>
                                                            <li><?php echo st_get_icon('eye-2', ['class' => 'me-1']); ?><?php echo esc_html(sprintf('%s views', $view)); ?></li>
                                                        <?php endif; ?>
                        ...
                        <div class="col-xxl-9 col-xl-8 col-lg-7">
                            <?php
                            if (empty($display_episode_data)) {
                                $display_episode_data = $episodes[0];
                            }
                            if ($display_episode_data) :
                                streamit_manage_views_count($display_episode_data);
                                streamit_get_template('episode/content/playlist_player.php', ['st_data' =>  $display_episode_data]);
                            ...
                            ?>
                                <div id="streamit_player_container" class="d-flex justify-content-between gap-4">
                                    <a href="<?php echo esc_url(streamit_get_permalink($display_episode_data->get_post_type(), $display_episode_data->get_post_name())); ?>">
                                        <h4 class="my-2 fw-bold"><?php echo esc_html($display_episode_data->get_post_title()); ?></h4>
                                    </a>
                                    <ul class="actions-playlist list-inline my-2 p-0 d-flex gap-2 justify-content-md-end">
                                        <?php if ($is_like_display && is_user_logged_in()) : ?>
                                            <li>
                                                <?php echo do_shortcode('[streamit_like_shortcode post_id="' . esc_attr($display_episode_data->get_id()) . '" post_type="' . esc_attr($display_episode_data->get_post_type()) . '"]'); ?>
                                            </li>
                                        <?php endif; ?>
                ...
                <?php else : ?>
                    <p class="no_data_found"><?php esc_html_e('No Episodes found.', 'streamit'); ?></p>
                <?php endif; ?>
            <?php else : ?>
                <p class="no_data_found"><?php esc_html_e('You cannot access this playlist.', 'streamit'); ?></p>
            <?php endif; ?>
        <?php else : ?>
            <p class="no_data_found"><?php esc_html_e('No Episode Playlist found.', 'streamit'); ?></p>
        <?php endif; ?>
    </div>
</div>