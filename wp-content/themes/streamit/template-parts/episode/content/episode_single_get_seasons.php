...
foreach ($seasons as $index => $val) {
    if (isset($val['episodes']) && is_array($val['episodes'])) {
        if (in_array($current_episode_id, $val['episodes'])) {
            $current_tab = $index;
            $slider_position = array_search($current_episode_id, $val['episodes']);
            $total_slides = count($val['episodes']);
        }
    }
}
                ...
                <div class="episode-season">
                        ...
                        <div class="item-list-tabs no-ajax css_prefix-tab-lists" id="object-nav">
                            ...
                            <ul class="nav nav-pills custom-tab-slider episode-nav-btn gap-3" id="nav-tab">
                                <?php foreach ($seasons as $index => $val) :
                                    $season_name_id = trim(preg_replace('/[^a-zA-Z0-9]+/', '_', strtolower($val['name'])), '-');
                                ...
                                ?>
                                    <li class="nav-item" role="presentation">
                                        <a class="nav-link init-episode-slider <?php echo $index === $current_tab ? 'active' : ''; ?>" data-display='<?php echo esc_attr($index); ?>' data-bs-toggle="pill" data-season_no="<?php echo esc_attr($index); ?>" href="#<?php echo esc_attr($season_name_id); ?>">
                                            <?php echo esc_html($val['name']); ?>
                                            <?php if ($is_upcoming_season && $enable_upcoming_badges) : ?>
                                                <span class="badge ms-2 upcoming-badge">
                    ...
                    <div class="tab-content mt-5" id="nav-tabContent">
                        ...
                        foreach ($seasons as $index1 => $season) :
                            ...
                            $is_upcoming_season = $season_upcoming_data['is_future_release'];
                        ?>
                            <div class="tab-pane fade  <?php echo $current_tab === $index1 ? ' active show' : ''; ?>" id="<?php echo trim(preg_replace('/[^a-zA-Z0-9]+/', '_', strtolower($season['name'])), '-'); ?>" role="tabpanel">
                                <div class="single_page_slick">
                                    <div class="css_prefix-episodes-content">
                                        ...
                                        if ($is_upcoming_season && !$is_admin) : ?>
                                            ...
                                            streamit_get_template('tvshow/content/tvshow_single_upcoming_season.php', ['season' => $season, 'season_upcoming_data' => $season_upcoming_data, 'st_data' => $tvshow, 'season_index' => $index1]);
                                            ?>
                                        <?php elseif (!empty($season['episodes'])) : ?>
                                            <div class=" css_prefix-slick-general st-skeleton" data-display='<?php echo esc_attr($index1); ?>' data-extra_settings="true" data-slider_settings="<?php echo st_get_slick_slider_settings($slick_args); ?>">
                                                <?php
                                                $episode_posts = streamit_get_episodes(['orderby' => 'post__in', 'include' => $season['episodes'],  'per_page' => -1])->results;
                                                foreach ($episode_posts as $index => $episode) :
                                                    streamit_get_template('episode/content/episode_single_slider.php', ['episode' => $episode , 'current_episode_id' => $current_episode_id]);
                                                endforeach;
                                            ?>
                                            </div>
                                        <?php else :
