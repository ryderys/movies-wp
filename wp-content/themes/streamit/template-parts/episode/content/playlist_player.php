...
$post_url     = streamit_get_permalink( 'episode', $post_slug );
...
$has_access = false;
if ( ! empty( $tvshow ) && function_exists( 'streamit_user_has_stream_access' ) ) {
    $has_access = streamit_user_has_stream_access( $tvshow_id, 'tvshow', get_current_user_id() );
}
...
$is_upcoming_data = function_exists( 'streamit_is_upcoming' ) ? streamit_is_upcoming( $st_data, 'episode' ) : [
    'is_upcoming' => false,
    ...
    <?php if ( $is_upcoming || current_user_can( 'administrator' ) ) : ?>
        ...
        <?php else : ?>
            <div class="restricted-block">
                <?php echo streamit_display_restricted_content( $tvshow, 'tvshow' ); ?>
            </div>
        <?php endif; ?>
