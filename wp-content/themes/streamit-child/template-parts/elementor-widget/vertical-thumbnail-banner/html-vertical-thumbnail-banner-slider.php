...
<div class="css_prefix-vertical-thumbnail-banner">
    ...
    foreach ($results as $post) {
        ...
        // --- Thumbnail slide (landscape nav strip) ---
        ob_start();
        $thumb_url = $image_id
            ? ( wp_get_attachment_image_url( $image_id, STREAMIT_CHILD_SIZE_GENRE ) ?: streamit_placeholder_image() )
            : streamit_placeholder_image();
        ?>
        <div class="slick-item">
            <div class="vertical-thumb-card position-relative">
                <a href="<?php echo esc_url(streamit_get_permalink($post_type, $post_slug)); ?>">
                    <img src="<?php echo esc_url($thumb_url); ?>" class="img-fluid w-100" alt="<?php echo esc_attr($post_title); ?>" loading="lazy" decoding="async">
                </a>
