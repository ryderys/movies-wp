...
$episode_has_access = function_exists('movies_wp_user_can_access_media')
    ? movies_wp_user_can_access_media(get_current_user_id())
    : ( !empty($tvshow) && function_exists('streamit_user_has_stream_access') && streamit_user_has_stream_access($tvshow_id, 'tvshow', get_current_user_id()) );

if ( ! $episode_has_access ) {
    $restrict_obj = !empty($tvshow) ? $tvshow : $st_data;
    $restrict_type = !empty($tvshow) ? 'tvshow' : 'episode';
    echo '<div class="restricted-block-episode">' . streamit_display_restricted_content($restrict_obj, $restrict_type) . '</div>';
    return;
}
